  ...
  >
    ...
    <td class="px-4 py-4 text-sm whitespace-nowrap" v-for="key in rowKeys.filter(k => k !== 'actions')" :key="key">
      ...
      <div v-else-if="key === 'dateTypeInfo'" class="flex flex-col gap-1.5">
        <div class="flex items-center gap-2">
          ...
          <div>
            <span class="text-xs text-gray-500 font-medium">Accepted:</span>
            <span class="text-sm font-medium text-gray-900 ml-1">
              {{ formatCellValue(key as any, row)?.acceptedDate || '—' }}
            </span>
          </div>
        </div>
        <div class="flex items-center gap-2">
    ...
    <td v-if="hasActions" class="px-4 py-4 text-start whitespace-nowrap">
      <RouterLink
        ...
        class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-gradient-to-r from-emerald-600 to-emerald-700 rounded-xl shadow-md transition-all duration-200 hover:shadow-lg hover:from-emerald-700 hover:to-emerald-800 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
        @click.stop
      >
