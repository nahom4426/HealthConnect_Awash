<!-- ApproveClaimRow.vue - Updated with institution name handling -->
<script setup>
...
function navigateToDetail(row) {
  if (row && row.claimUuid) {
    ...
    router.push({
      path: `/approve_claims/detail/${row.claimUuid}`,
...
<template>
  ...
  >
    ...
    <td class="p-3 py-4" v-for="key in rowKeys" :key="key">
      ...
      <!-- Claim Level Column -->
      <div v-else-if="key === 'claimLevel'" class="flex flex-col">
    ...
    <td class="p-3">
      <div class="flex items-center gap-2">
        <Button 
          size="xs" 
          type="elevated" 
          class="px-3 py-1.5 text-xs font-medium text-white bg-gradient-to-r from-purple-600 to-purple-700 rounded-lg hover:from-purple-700 hover:to-purple-800 transition-all duration-200 shadow-md hover:shadow-lg"
