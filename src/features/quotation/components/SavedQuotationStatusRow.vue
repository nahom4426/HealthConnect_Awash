...
function handleViewWithClose(row: any) {
  ...
  // Get institution ID - can be null for individual policies
  const institutionId = row?.institutionId ?? row?.institutionUuid ?? 'individual';
...
<template>
  ...
  >
    ...
    <!-- Data Columns -->
    <td class="px-4 py-4 text-sm whitespace-nowrap" v-for="key in (rowKeys as any[]).filter((k: any) => k !== 'actions')" :key="key">
      <!-- Status Badge -->
      ...
      <div v-else-if="key === 'dateTypeInfo'" class="flex flex-col gap-1.5">
        <div class="flex items-center gap-2">
          ...
          <div>
            <span class="text-xs text-gray-500 font-medium">Created:</span>
            <span class="text-sm font-medium text-gray-900 ml-1">
              {{ formatCellValue(key as any, row)?.createdDate || '—' }}
            </span>
          </div>
        </div>
        <div class="flex items-center gap-2">
    ...
    <td
      class="px-4 py-4 text-start whitespace-nowrap"
      v-if="(headKeys as any[]).includes('Actions') || (headKeys as any[]).includes('actions')"
    >
      <button
        @click.stop="handleViewWithClose(row)"
        class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-gradient-to-r from-amber-600 to-amber-700 rounded-xl shadow-md transition-all duration-200 hover:shadow-lg hover:from-amber-700 hover:to-amber-800 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-amber-500/50"
      >
