<!-- ApproveClaimDetail.vue - Updated with institution name handling -->
<script setup>
...
// Log for debugging
console.log('Approve Claim Detail - Institution name:', institutionName.value);
console.log('Approve Claim Detail - Route state:', route.state);
console.log('Approve Claim Detail - Route query:', route.query);
</script>
      ...
      <div class="flex items-center justify-between w-full">
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center text-white font-bold shadow-lg">
              {{ getInitials(institutionName) }}
            </div>
            <div>
              <h1 class="text-xl font-bold text-gray-800">{{ institutionName }}</h1>
              <p class="text-sm text-gray-500">Approve Claim Details</p>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2 px-3 py-1.5 bg-indigo-50 rounded-lg border border-indigo-200">
            <span class="text-xs font-medium text-indigo-600">Status:</span>
            <span class="text-xs font-semibold text-indigo-700">CONFIRMED</span>
          </div>
        </div>
      </div>
    </template>
