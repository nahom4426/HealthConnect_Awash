...
function confirmAdd() {
  ...
  api.send(
    ...
    (res) => {
      if (res.success) {
        window.dispatchEvent(new CustomEvent('provider-mapping-changed', { detail: { action: 'add' } }));
...
<template>
  ...
  <ModalParent id="confirm-add-provider-modal">
    ...
    >
      <div class="space-y-6">
        <div class="p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-100">
          <div class="flex gap-4 items-center">
            <div class="flex justify-center items-center w-12 h-12 bg-blue-500 rounded-lg">
        ...
        <div class="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
          <div class="flex gap-3 items-start">
            <div class="mt-0.5 w-5 h-5 text-yellow-600">
      ...
      <template #bottom>
        <div class="flex gap-4 justify-end items-center p-2 m-2 w-full">
          <Button
            @click="closeModal"
            variant="outline"
          ...
          >
            <i v-html="icons.plus || '+'" class="mr-2 w-4 h-4"></i>
            Add Provider
          </Button>
        </div>
      </template>
