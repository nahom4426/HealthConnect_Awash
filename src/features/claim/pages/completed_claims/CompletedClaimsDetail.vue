...
// ─── Currency helper ──────────────────────────────────────────────────────────
// Replace your existing formatCurrency util calls with this wrapper so all
// amounts in this export show "ብር" instead of "e-"
...
// ─── Column layout (landscape A4 = 841.89 pt, margins 12+12 = 797.89 usable) ─
//
//  #             20
//  Provider      65
//  Insured/Dep   90   ← merged Insured Name + Dependant Name
//  Provided Date 52
//  Amount        80
//  Item Code     38
//  Item Name     95
...
//  Excess Used   60
//                ─────
//  TOTAL:       736  (fits comfortably within 797.89 pt usable width)
...
const { exporting: exportingPdf, exportPdf } = useExportPdf({
  ...
  // ── 13 columns (removed: Institution Name, Service Status, Person Type, Unit)
  // ── Insured Name + Dependant Name merged into "Insured / Dependant"
  headers: [
    '#',
    'Provider Name',
    'Insured / Dependant',   // ← merged column
    'Provided Date',
    'Amount',
    'Item Code',
    'Item Name',
  ...
  // ── Column widths — total 736 pt, fits landscape A4 with 12pt margins ──
  columnStyles: {
    0:  { cellWidth: 20 },   // #
    1:  { cellWidth: 65 },   // Provider Name
    2:  { cellWidth: 120 },   // Insured / Dependant
    3:  { cellWidth: 52 },   // Provided Date
    4:  { cellWidth: 90 },   // Amount
    5:  { cellWidth: 38 },   // Item Code
    6:  { cellWidth: 95 },   // Item Name
    7:  { cellWidth: 42 },   // Item Type
    8:  { cellWidth: 26 },   // Qty
    9:  { cellWidth: 68 },   // Unit Price
    10: { cellWidth: 70 },   // Total Price
    11: { cellWidth: 70 },   // Excess Used
  },

  // ── Merge first 5 columns across sub-rows of the same claim ──
  ...
  mapRowData: (row, item, isFirstItem, rowIndex) => {
    ...
    // Merge insured + dependant into one field
    const insuredAndDep = (() => {
    ...
    return [
      ...
      isFirstItem ? (row.providerName    || 'N/A')  : '',  // 1  Provider Name
      isFirstItem ? insuredAndDep                   : '',  // 2  Insured / Dependant
      isFirstItem ? providedDate                    : '',  // 3  Provided Date
      isFirstItem ? formatBirr(row.amount || 0)     : '',  // 4  Amount
      item?.itemCode   || 'N/A',                           // 5  Item Code
      item?.itemName   || 'N/A',                           // 6  Item Name
      item?.itemType   || 'N/A',                           // 7  Item Type
      item?.quantity   ?? 0,                               // 8  Qty
      formatBirr(item?.unitPrice  || 0),                   // 9  Unit Price
      formatBirr(item?.totalPrice || 0),                   // 10 Total Price
  ...
  <template>
    <DefaultPage>
     ...
     <!-- <div class="p-5 mb-6 bg-gradient-to-r from-gray-50 to-white rounded-xl border border-gray-200 shadow-sm">
        <div class="flex flex-col gap-4 md:flex-row md:justify-between md:items-center">
          <div class="flex-1">
            <div class="flex items-center mb-3">
              <div class="p-2 bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-lg border border-indigo-200">
                <div v-html="icons.collection" class="w-5 h-5 text-indigo-600"></div>
              </div>
              <div class="ml-3">
                <h3 class="text-lg font-bold text-gray-900">Batch Details</h3>
                <div class="flex flex-wrap gap-3 mt-2">
                  <div class="inline-flex gap-2 items-center">
                    <span class="text-sm font-medium text-gray-600">Batch Code:</span>
                    <span class="px-3 py-1.5 text-sm font-semibold text-indigo-700 bg-gradient-to-r from-indigo-50 to-indigo-100 rounded-lg border border-indigo-200">
                      {{ batchCode }}
                    </span>
                  </div>
                  <div class="inline-flex gap-2 items-center">
                    <span class="text-sm font-medium text-gray-600">Claim ID:</span>
                    <span class="px-3 py-1.5 text-sm font-mono bg-gradient-to-r from-gray-50 to-gray-100 text-gray-700 rounded-lg border border-gray-200 truncate max-w-[200px]">
                      {{ claimUuid.slice(0, 8) }}...
                    </span>
                  </div>
                </div>
            ...
            <div class="mt-4">
              <div class="flex justify-between mb-1 text-sm text-gray-600">
                ...
                <span>{{ Math.round((totals.processedCount / totals.totalCount) * 100) || 0 }}%</span>
              </div>
              <div class="overflow-hidden h-2 bg-gray-200 rounded-full">
                <div 
                  ...
                  :style="{ width: `${(totals.processedCount / totals.totalCount) * 100 || 0}%` }"
                ></div>
              </div>
            </div>
          </div>
          ...
          <Button
            v-if="canProcessWholeClaim"
            @click="openProcessWholeClaim"
            type="elevated"
            size="lg"
            class="text-white bg-gradient-to-r from-green-500 to-emerald-600 shadow-lg hover:from-green-600 hover:to-emerald-700 shadow-emerald-200/50"
          >
            <div v-html="icons.checkBadge" class="mr-2 w-5 h-5"></div>
            Check Entire Batch
          </Button>
        </div>
      </div> -->
