...
<template>
	<DefaultPage>
		<template #more >
			<Button :pending="processedClaimReq.pending.value" class="ml-auto" @click="batchVerified" type="primary" v-if="checked.length" >
				Verify Selected
			</Button>
		</template>
		<TableWithCheckBox
		...
		>
			<template #actions="{ row }" >
				<Button size="xs" type="elevated" >
					<RouterLink :to="`/verify_claims/detail/${providerUuid}/${encodeURIComponent(batchCode as string)}/individual/${row?.claimUuid}`" >
						Detail
					</RouterLink>
				</Button>
			</template>
