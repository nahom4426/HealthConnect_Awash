          ...
          <Button size="xs" type="elevated">
            <RouterLink
              :to=" ServiceTypes.creditService == active ?
               `/approve_claims/detail/${(row as BatchClaim).providerUuid}/${encodeURIComponent((row as BatchClaim).batchCode)}` :
               `/approve_claims/cash_detail/${encodeURIComponent((row as CashBatchClaim)?.claimBatchCode)}`"
            >
              Detail
            </RouterLink>
          </Button>
