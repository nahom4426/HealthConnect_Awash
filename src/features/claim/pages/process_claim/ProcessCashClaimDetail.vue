...
import { claimProccessed, getRequestedCashClaimByBatchDetail, getRequestedClaimByBatchDetail } from '../../api/claimApi';
...
const pagination = usePagination({
	cb: (data: any) => getRequestedCashClaimByBatchDetail({
		...data,
		...
		>
			<template #actions="{ row }" >
				<Button size="xs" type="elevated" >
					<RouterLink :to="`/process_claims/cash_detail/${encodeURIComponent(batchCode as string)}/individual/${row?.claimUuid}`" >
						Detail
					</RouterLink>
				</Button>
			</template>
