...
function handleSubmit() {
  ...
  if (selected.length === 0) {
    toasted(false, "", "Only Individual Plan packages can be saved for dependents");
    return;
  }
  ...
  apiRequest.send(
    () => {
      const query = {
        ...
        insuredUuid: props.data.dependantUuid || props.data.insuredUuid,
    ...
    (res) => {
      if (res.success) {
        toasted(res.success, "Packages added to dependent successfully!", res.error);
        closeModal();
      }
    }
  );
...
<template>
  <ModalParent>
    <NewFormParent
      size="lg"
      title="Add Benefit Packages to Dependent"
      subtitle="Select packages and set sum assured for this dependent"
    >
      <div class="overflow-y-auto overflow-x-hidden form-scrollbar">
        <Form
          class="p-6 space-y-6"
          id="dependentPackagesForm"
          v-slot="{ submit }"
